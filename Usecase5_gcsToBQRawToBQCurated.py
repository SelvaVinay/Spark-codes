#Note (Prerequisite): Please download the code from the below gcs bucket (gs://incpetez-data-samples/code/Usecase5_gcsToBQRawToBQCurated.py) and modify the buckets with your bucket where ever write is happening. For eg:  change this bucket 'incpetez-data-samples/tmp' to your bucket 
#prerequisites
...
def main():
   ...
   gcs_df=spark.read.csv("gs://mydata-store/mock-data/MOCK_DATA.csv",sep=',',inferSchema=True,header=True)\
   .toDF("customer_id","first_name","last_name","age","email","country","postal_code","gender","phone_number","account_balance","last_login","active_customer")
   gcs_df.show()
   ...
   .option("temporaryGcsBucket",'mydata-store/tmp')\
   .option('table', 'rawds.cust_raw') \
   .save()
   ...
   curated_bq_df=spark.sql("""select customer_id,concat(first_name,',',last_name) as Full_Name,age,email,gender,
   country,coalesce(postal_code,'-1') as postal_code ,phone_number,coalesce(account_balance,'0') as account_balance,last_login,active_customer from raw_view where active_customer = false """)
   ...
   .option("temporaryGcsBucket",'mydata-store/tmp')\
   .option('table', 'curatedds.cust_curated') \
   .save()
